// BlockMaskInteractive.tsx
import { useEffect, useRef } from "react";
import {
  Application,
  Graphics,
  Sprite,
  DisplacementFilter,
  Filter,
  Texture,
  Assets,
} from "pixi.js";

type Props = {
  /** размер фигуры как в макете (1 = исходный, 1.1 = 110%) */
  scale?: number;
  /** сдвиг фигуры вниз, в процентах от высоты экрана */
  shiftYPercent?: number;
  /** бежевый цвет заливки */
  color?: number;
  /** сила вмятия (px) */
  strength?: number;
  /** радиус влияния курсора (px) */
  radius?: number;
};

const PATH_D =
  "M144.374 224.487C159.266 198.694 191.41 202.504 210.359 203.657C343.091 211.577 440.587 141.919 528 50C625.464 -50.8645 780.296 31.8756 903.609 39.1829C1048.76 47.8151 1329.97 1.11254 1369.68 24.0363C1433.05 60.6261 1466.38 73.0741 1466.38 73.0741C1513.57 100.318 1510.16 167.47 1513.62 169.469C1528.34 177.962 1559.65 174.974 1579.3 186.319C1615.14 207.009 1570.88 256.283 1623.2 286.496C1649.22 301.514 1673.09 280.974 1687.98 255.181C1693.37 245.853 1697.97 236.003 1703.15 227.032C1723.07 192.523 1752.8 189.704 1779.88 205.339C1820.75 228.938 1852.35 295.774 1879.94 311.703C1896.99 321.548 1919.5 324.759 1936.1 334.339C1947.19 340.746 1960.07 353.823 1953.6 365.038C1948.54 373.805 1936.46 382.261 1932.37 389.346C1928.01 396.891 1938.5 404.237 1946.03 408.586C1989.25 433.538 2040.04 363.087 2081.93 387.275C2083.71 388.303 2169.36 498.713 2172.7 516.548C2173 517.673 2172.78 518.767 2172.25 519.685C2172.25 519.685 2276.7 614.789 2301.87 708.975C2302.86 724.298 2288.52 740.554 2289.52 755.877C2290.49 815.909 2321.29 896.352 2290.5 949.67C2278.79 969.958 2252.72 985.964 2251.04 988.87C2234.2 1018.03 2279.63 1070.08 2252.26 1117.48C2231.04 1154.24 2176.19 1165.66 2139.79 1185.08C2070.64 1230.8 1966.28 1372.88 1892.01 1330C1881.78 1324.09 1871.47 1316.78 1860.68 1310.55C1839.1 1298.09 1809.34 1288.04 1796.86 1309.65C1791.03 1319.75 1794.03 1330.65 1808.44 1338.97C1830.68 1351.81 1865.4 1350.58 1878.33 1358.05C1888.3 1363.81 1880.44 1384.01 1874.67 1394C1856.04 1426.27 1812.6 1448.49 1791 1485.9C1764.63 1531.58 1797.07 1538.88 1774.49 1577.98C1757.04 1608.21 1718.09 1615.49 1705.32 1637.61C1704.34 1639.29 1701.52 1698.02 1682.34 1731.25C1676.1 1742.06 1654.97 1736.93 1646.16 1731.84C1630.99 1723.09 1622.68 1702.92 1609.14 1695.11C1598.9 1689.2 1562.73 1724.46 1517.02 1698.06C1495.03 1685.37 1475.26 1663.89 1452.3 1650.64C1437.74 1642.23 1423.02 1634.82 1415.28 1648.23C1403.98 1667.8 1482.06 1725.59 1501.66 1736.9C1566.81 1774.52 1596.51 1747.42 1574.85 1784.94C1556.54 1816.65 1528.83 1784.94 1500.35 1783.25C1430.06 1781.21 1107.6 1919.7 981.914 1847.14C889.625 1793.85 933.137 1730.01 791.013 1647.95C777.523 1640.17 764.005 1632.43 750.617 1624.7C569.297 1520.01 494.106 1408.43 506.849 1386.36C512.735 1376.17 538.88 1372.03 547.562 1356.99C552.506 1348.43 550.302 1345.66 545.772 1343.04C519.2 1327.7 469.977 1413.19 421.466 1385.19C334.725 1335.11 374.244 1136.29 327.998 1027.82C301.477 963.374 238.967 927.284 190.746 874.908C152.998 838.366 139.04 786.13 103.411 745.918C-54.3195 582.741 23.7775 657.667 24.7011 474.558C24.6613 417.376 37.9009 353.181 67.5956 301.749C86.2191 269.139 111.614 241.73 144.374 224.487Z";

export default function BlockMaskInteractive({
  scale = 1.1,
  shiftYPercent = 12,
  color = 0xf6eedd,
  strength = 26,
  radius = 160,
}: Props) {
  const hostRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const host = hostRef.current;
    if (!host) return;

    let destroyed = false;

    (async () => {
      const app = new Application();
      await app.init({
        resizeTo: host,
        backgroundAlpha: 0, // прозрачный канвас
        antialias: true,
      });
      app.canvas.style.background = "transparent";
      app.canvas.style.position = "absolute";
      app.canvas.style.inset = "0";
      app.canvas.style.zIndex = "9";
      app.canvas.style.pointerEvents = "none";

      if (destroyed) return;
      host.appendChild(app.canvas);
      app.canvas.style.display = "block";

      // 1) Оверлей: будем рисовать БЕЖЕВЫЙ фуллскрин и «выпиливать» из него SVG-фигуру
      const overlay = new Graphics();
      app.stage.addChild(overlay);

      // 2) Displacement карта (для мягкого «прогиба» у курсора)
      const mapUrl = createRadialMap(radius);
      const dispTex: Texture = await Assets.load(mapUrl);
      const disp = new Sprite(dispTex);
      disp.anchor.set(0.5);
      disp.position.set(app.renderer.width / 2, app.renderer.height / 2);
      app.stage.addChild(disp);

      const dFilter = new DisplacementFilter(disp);
      dFilter.padding = radius * 2;
      dFilter.scale.set(0, 0); // включим при первом движении
      overlay.filters = [dFilter as unknown as Filter];
      overlay.filterArea = app.screen;

      let activated = false;

      const draw = () => {
        const W = app.renderer.width;
        const H = app.renderer.height;

        // центр исходного SVG
        const cx = 2309 / 2;
        const cy = 1877 / 2;
        const ty = (shiftYPercent / 100) * H;

        // применяем transform прямо на SVG-группу
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2309 1877">
  <g transform="translate(${W / 2}, ${
          H / 2 + ty
        }) scale(${scale}) translate(${-cx}, ${-cy})">
    <path d="${PATH_D}"/>
  </g>
</svg>`;

        // перерисовка: бежевый прямоугольник → вырезаем SVG → готовая «прозрачная дыра»
        overlay.clear();
        overlay.rect(0, 0, W, H).fill(color);
        overlay.svg(svg).cut(); // ВОТ ЗДЕСЬ «выпил» — дырка становится ПРОЗРАЧНОЙ

        if (!activated) disp.position.set(W / 2, H / 2);
      };

      draw();
      app.renderer.on("resize", draw);

      // движение курсора = двигаем карту смещений (прогибаем край)
      const onMove = (e: MouseEvent) => {
        const r = host.getBoundingClientRect();
        disp.position.set(e.clientX - r.left, e.clientY - r.top);
        if (!activated) {
          dFilter.scale.set(strength, strength);
          activated = true;
        }
      };
      host.addEventListener("mousemove", onMove);

      // cleanup
      (host as unknown as { __pixiCleanup?: () => void }).__pixiCleanup =
        () => {
          host.removeEventListener("mousemove", onMove);
          app.destroy(true, { children: true });
        };
    })();

    return () => {
      destroyed = true;
      (
        hostRef.current as unknown as { __pixiCleanup?: () => void }
      )?.__pixiCleanup?.();
    };
  }, [scale, shiftYPercent, color, strength, radius]);

  return (
    <div
      ref={hostRef}
      style={{
        position: "absolute",
        inset: 0,
        pointerEvents: "auto",
      }}
    />
  );
}

/** Генерим dataURL для displacement-карты (центр чуть темнее 128 → тянет внутрь) */
function createRadialMap(size: number): string {
  const s = Math.max(64, Math.round(size * 2));
  const c = document.createElement("canvas");
  c.width = s;
  c.height = s;
  const ctx = c.getContext("2d")!;
  ctx.fillStyle = "rgb(128,128,128)";
  ctx.fillRect(0, 0, s, s);
  const g = ctx.createRadialGradient(s / 2, s / 2, 0, s / 2, s / 2, s / 2);
  g.addColorStop(0, "rgb(110,110,110)");
  g.addColorStop(0.7, "rgb(128,128,128)");
  g.addColorStop(1, "rgb(128,128,128)");
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.arc(s / 2, s / 2, s / 2, 0, Math.PI * 2);
  ctx.fill();
  return c.toDataURL("image/png");
}
